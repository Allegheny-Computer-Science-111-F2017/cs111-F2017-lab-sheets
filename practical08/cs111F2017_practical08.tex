...
in two different triangular shapes, in adherence to the symbol count read in from the file. Notice that the first
triangle in exactly the same as the one produced by a program that we studied during a classroom session. Creating the
second triangle will require you to write \program{for} loops that create a triangle that ``mirrors`` the first one.
Finally, make sure that you have correct formatting and documentation in both of these files; check the {\tt README.md}
file for a statement of other checks.
